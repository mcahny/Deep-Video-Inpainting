...
class VI_2D_Encoder_3(nn.Module):
    def __init__(self, opt):
        ...
        ### ENCODER
        st = 2 if self.opt.half_size else 1
...
class VI_2D_Decoder_3(nn.Module):
    def __init__(self, opt):
        ...
        self.opt=opt
        dv = 2 if self.opt.half_size else 1
        ### decoder
    ...
    def forward(self, x, x2_64_warp=None, x2_128_warp=None):
        ...
        # FULL SCALE M3S
        if self.opt.multi_scale and x2_64_warp is not None and x2_128_warp is not None:
...
class VINet_Hexa_3_M3S(nn.Module):
    ...
    def forward(self, masked_img, mask, prev_state=None, prev_feed=None, prev_state2=None, idx=0): # masked img: b x C x TxHxW
        T = masked_img.size(2)
        ref_idx = (T-1)//2 # 2 --> 0
        ...
        if self.opt.half_size and flow6_256 is not None and flow6_512 is not None:
            flow6_256 = flow6_512
        if not self.opt.no_train:
            if self.opt.prev_warp and prev_state is not None:
                if self.opt.loss_on_raw:
                    return (output, output_), torch.stack([flow2_128,flow3_128,flow4_128,flow5_128, flow6_128],2), state, None, torch.stack([occ_mask, 1-occ_mask, occ_mask_64, 1-occ_mask_64, occ_mask_128, occlusion_mask_256], 2), flow6_256
                else:
