...
opt.pretrain_path = 'results/vinet_agg_rec/save_6.pth'
...
opt.no_train = True
opt.test = True
opt.multi_scale = True # Always True
opt.coherent = True # Always True
opt.t_stride = 3

opt.prev_warp = True
opt.output_clamp = True
opt.reverse = False
opt.save_image = True
...
with torch.no_grad():
    for seq, (inputs, masks, info) in enumerate(DTloader):
        ...
        for t in range(num_frames):
            ...
            if t == 0:
                outputs, flows, lstm_state, _, occ_masks = model(masked_inputs_, masks_, lstm_state, prev_feed, None)
            else:
                outputs, flows, lstm_state, _, occ_masks, flow_256 = model(masked_inputs_, masks_, lstm_state, prev_feed, None, t)
            lstm_state = None
            end = time.time() - start
        ...
        print('Finishied {}, total inference time is {}'.format(seq_name, total_time))
        if opt.save_video:
